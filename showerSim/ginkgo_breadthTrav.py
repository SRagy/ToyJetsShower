...
def _traverse(self, root, inputs, fixed=True, delta_P=None, cut_off=None, rate=None, suppress_output=False):
    ...
    while q:
        ...
        inv_mass = root[0]**2 - (root[1]**2 + root[2]**2 + root[3]**2)
...
def _split(self,
    ...
    """
    Function to make the jet tree.
    """

    # The tree structure may not always contain the same nodes. If fixed = True, this is 
    # accounted for by ensuring that a given node will always refer to a given position in the
    # inputs list (these inputs will not be expended if the node is not present). Otherwise,
    # all inputs are consumed as they come.
    
    if fixed:
        start_index = 4*idx
    else:
        start_index = 4*self.node_counter
    ...
    if delta_P > cut_off:
        ...
        q.append([2*idx+1, pL_mu])
        q.append([2*idx+2, pR_mu])
        
        draws.append(draw_decay_L)
        draws.append(draw_decay_R)
...
class SimulatorModel(PyprobSimulator):
    def __init__(self, control=None, fixed=True,jet_p=None, pt_cut=1.0, M_hard=None, Delta_0=None, minLeaves=None, maxLeaves=None,
    ...
    def forward(self, inputs=None):
        ...
        jet = dict()
        jet["root_id"] = 0
        jet["tree"] = to_tensor(tree) # Labels for the nodes in the tree
