...
def _traverse_rec(
    ...
    if delta_P > cut_off:
        ...
        phi_CM = pyprob.sample(pyprob.distributions.Uniform(0, 2 * np.pi), name="phiCM" + str(idx) + str(is_left), control=False)
        theta_CM_U = pyprob.sample(pyprob.distributions.Uniform(0, 1), name="thetaCM_U" + str(idx) + str(is_left), control=False)        
